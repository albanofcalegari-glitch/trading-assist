"""
generate_nu_pdf.py  — replica fiel de NU estilo TradingView dark.

Enfoque:
  - mplfinance SOLO para velas + volumen (sin addplot)
  - canal dibujado 100% manual con ax.plot() en coords enteras
  - fill_between con interpolacion continua (sin huecos)
  - linea media en escala log (promedio geometrico)
  - proyeccion 6 meses al futuro
  - etiqueta precio actual lado derecho
"""
...
NU_ID       = 7783
DAYS_BACK   = 1800
FUTURE_DAYS = 130        # ~6 meses de dias habiles proyectados
OUTPUT_PDF  = r'C:\Users\DELL\Desktop\NU_canal.pdf'

# ── Colores exactos TradingView dark ──────────────────────────────────────────
TV_BG        = '#131722'
TV_GRID      = '#1e222d'
TV_BORDER    = '#2a2e39'
TV_TEXT      = '#b2b5be'
TV_UP        = '#26a69a'
TV_DOWN      = '#ef5350'
CH_LINE      = '#00897b'   # teal del canal (lineas solidas)
CH_MID       = '#26a69a'   # linea media (ligeramente mas clara)
CH_FILL      = '#00897b'
CH_ALPHA     = 0.10
CH_LW        = 2.4         # grosor de las lineas del canal
...
def add_future_bars(df: pd.DataFrame, n_days: int) -> pd.DataFrame:
    """Agrega n_days dias habiles de NaN para proyectar las lineas a futuro."""
    last = df.index[-1]
    future = []
    d = last
...
def find_trendline(df_orig: pd.DataFrame, kind: str):
    """
    Retorna (ls, li, start_idx, piv_dates, piv_vals) o None.
    ls/li = pendiente/intercepto en espacio log.
    precio en barra i = exp(ls*i + li)
    """
    n    = len(df_orig)
    ...
    # Filtro: el precio debe reaccionar >=2% en las 15 barras siguientes
    sig = []
    for (i, d, v, lv) in raw:
    ...
    # Mejor par por score: toques*3 - violaciones*5 + bonus_extension
...
def eval_line(ls: float, li: float, start_idx: int, n_total: int) -> np.ndarray:
    """
    Evalua la linea en todos los n_total puntos (np.nan antes de start_idx).
    Retorna array de shape (n_total,).
    """
...
def build_channel(sup_result, res_result, n_total):
    """
    Retorna (sup_y, res_y, mid_y, x_fill, y_fill_lo, y_fill_hi)
    Todos los arrays tienen longitud n_total (NaN donde no aplica).
    """
    sup_y = eval_line(*sup_result[:3], n_total) if sup_result else None
    res_y = eval_line(*res_result[:3], n_total) if res_result else None
...
# ── Dibujo del canal sobre los ejes de mplfinance ────────────────────────────

def draw_channel(ax, sup_y, res_y, mid_y, n_total):
    """
    Dibuja canal con coordenadas enteras (eje X de mplfinance = 0..n-1).
    """
    x_all = np.arange(n_total)

    # Lineas solidas sup y res
    for y_arr, lw in [(sup_y, CH_LW), (res_y, CH_LW)]:
        if y_arr is None: continue
        mask = np.isfinite(y_arr)
        x_v  = x_all[mask]
        y_v  = y_arr[mask]
        if len(x_v) >= 2:
            ax.plot(x_v, y_v, color=CH_LINE, linewidth=lw,
                    solid_capstyle='round', zorder=8)

    # Relleno del canal (interpolacion continua para evitar huecos)
    if sup_y is not None and res_y is not None:
        both = np.isfinite(sup_y) & np.isfinite(res_y)
        if both.sum() >= 2:
            x_fill = x_all[both]
            y_lo   = np.minimum(sup_y[both], res_y[both])
            y_hi   = np.maximum(sup_y[both], res_y[both])
            ax.fill_between(x_fill, y_lo, y_hi,
                            alpha=CH_ALPHA, color=CH_FILL,
    ...
    if mid_y is not None:
        ...
        y_m  = mid_y[mask]
        if len(x_m) >= 2:
            ax.plot(x_m, y_m, color=CH_MID, linewidth=1.1,
                    linestyle=(0, (6, 5)),   # patron dash personalizado
                    alpha=0.80, zorder=7)

    # Pivotes (circulos en los puntos de anclaje)
    # Los recuperamos del caller


def draw_pivot(ax, df_ext_index, piv_dates, piv_vals, color):
    for d, v in zip(piv_dates, piv_vals):
        try:
            pos = df_ext_index.get_loc(d)
            ax.plot(pos, v, 'o', color=color, ms=7, zorder=12,
                    mec=TV_BG, mew=1.5)
        except Exception:
            pass


# ── Ticks del eje Y al estilo TradingView (escala log) ────────────────────────

def set_log_yticks(ax, y_lo, y_hi):
    """Genera ticks en valores 'redondos' tipicos de TradingView en log."""
    sequence = [
        0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50,
        0.60, 0.70, 0.80, 0.90, 1.00, 1.10, 1.20, 1.30, 1.50,
        1.80, 2.00, 2.20, 2.60, 3.00, 3.10, 3.50, 3.70,
        4.00, 4.50, 5.00, 5.50, 6.00, 6.50, 7.00, 8.00, 9.00,
        10, 11, 12, 14, 16, 18, 20, 22, 25, 28, 32, 36, 40, 45, 50,
        55, 60, 70, 80, 90, 100, 120, 150, 200,
    ]
    ...
    ax.yaxis.set_major_formatter(
        mticker.FuncFormatter(lambda x, _: f'{x:.2f}' if x < 5 else f'{x:.0f}')
    )
...
def main():
    print('Cargando datos NU...')
    df_orig = fetch_ohlcv(NU_ID)
    if df_orig.empty:
        print('Sin datos'); return

    n_orig     = len(df_orig)
    last_close = float(df_orig['close'].iloc[-1])
    print(f'  {n_orig} velas  |  ultimo cierre: ${last_close:.2f}')

    # Trendlines (sobre datos originales, sin NaN futuros)
    sup = find_trendline(df_orig, 'support')
    res = find_trendline(df_orig, 'resistance')
    print(f'  Soporte:     {"OK" if sup else "NO encontrado"}')
    print(f'  Resistencia: {"OK" if res else "NO encontrada"}')

    # Extender df con dias futuros
    df_ext = add_future_bars(df_orig, FUTURE_DAYS)
    n_ext  = len(df_ext)

    # Construir arrays del canal para n_ext puntos
    sup_y, res_y, mid_y = build_channel(sup, res, n_ext)

    # ── Grafico base: velas + volumen (SIN addplot de canal) ──────────────────
    ...
    ax     = axes[0]   # panel precio
    ax_vol = axes[2]   # panel volumen

    # ── Canal manual (coordenadas enteras 0..n_ext-1) ─────────────────────────
    draw_channel(ax, sup_y, res_y, mid_y, n_ext)

    # Pivotes
    if sup: draw_pivot(ax, df_ext.index, sup[3], sup[4], CH_LINE)
    if res: draw_pivot(ax, df_ext.index, res[3], res[4], CH_LINE)

    # ── Eje Y: ticks log estilo TradingView, a la derecha ─────────────────────
    ...
    xlim = ax.get_xlim()
    # Linea horizontal punteada al precio actual
    ax.axhline(y=last_close, color=TV_TEXT, lw=0.6, linestyle='--',
               alpha=0.45, zorder=6)
    # Badge con precio (clipear fuera del axes para que aparezca sobre el borde)
    ax.annotate(
        f'  {last_close:.2f}  ',
    ...
    handles = []
    if sup_y is not None and res_y is not None:
        v_sup = sup_y[np.isfinite(sup_y)][-1]
        v_res = res_y[np.isfinite(res_y)][-1]
        handles += [
    ...
    # ── Titulo ────────────────────────────────────────────────────────────────
    ax.set_title(
        'NU  ·  Nu Holdings Ltd.  ·  1D  ·  NYSE  ·  Log',
    ...
    with PdfPages(OUTPUT_PDF) as pdf:
        info = pdf.infodict()
        info['Title'] = 'NU Holdings - Canal Dinamico Diario'
        pdf.savefig(fig, facecolor=TV_BG, dpi=150, bbox_inches='tight')
    plt.close(fig)

    print(f'\nChecklist:')
    print(f'  [OK] Escala logaritmica')
    print(f'  [OK] Velas japonesas')
    print(f'  [OK] Volumen abajo')
    print(f'  [{"OK" if sup_y is not None else "NO"}] Linea inferior del canal')
    print(f'  [{"OK" if res_y is not None else "NO"}] Linea superior del canal')
    print(f'  [{"OK" if mid_y is not None else "NO"}] Linea media punteada')
    print(f'  [{"OK" if sup_y is not None and res_y is not None else "NO"}] Sombreado interno')
    print(f'  [OK] Estilo TradingView dark')
