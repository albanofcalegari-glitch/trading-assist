...
import type { TieredSRResponse } from '@/lib/api'
...
interface Props {
  ...
  // Tiered S/R behavioral
  tieredSR?:                TieredSRResponse | null
}
...
export default function PriceChart({
  ...
  useEffect(() => {
    ...
    // ── Tiered S/R behavioral ──────────────────────────────────────────────────
    if (tieredSR && data.length >= 2) {
      const tierDefs: { tier: 'historical' | 'accelerated' | 'tactical'; side: 'support' | 'resistance'; color: string; width: 1 | 2; style: number }[] = [
        { tier: 'historical',  side: 'support',    color: 'rgba(34,197,94,0.7)',   width: 2, style: LineStyle.Solid },
        { tier: 'historical',  side: 'resistance', color: 'rgba(239,68,68,0.7)',   width: 2, style: LineStyle.Solid },
        { tier: 'accelerated', side: 'support',    color: 'rgba(163,230,53,0.65)', width: 2, style: LineStyle.Dashed },
        { tier: 'accelerated', side: 'resistance', color: 'rgba(251,146,60,0.65)', width: 2, style: LineStyle.Dashed },
        { tier: 'tactical',    side: 'support',    color: 'rgba(45,212,191,0.55)', width: 1, style: LineStyle.Dotted },
        { tier: 'tactical',    side: 'resistance', color: 'rgba(251,113,133,0.55)',width: 1, style: LineStyle.Dotted },
      ]

      for (const td of tierDefs) {
        const tierData = tieredSR[td.side]?.[td.tier]
        if (!tierData || !tierData.line_points?.length) continue

        const pts = firstCandleDate
          ? tierData.line_points.filter((p: any) => p.fecha >= firstCandleDate)
          : tierData.line_points
        if (pts.length < 2) continue

        const series = addOverlay({
          color:                  td.color,
          lineWidth:              td.width as any,
          lineStyle:              td.style as any,
          priceLineVisible:       false,
          lastValueVisible:       false,
          crosshairMarkerVisible: false,
        })
        series.setData(pts.map((p: any) => ({ time: p.fecha as any, value: p.value })))
      }
    }
  ...
  }, [candles, freq, indicators, showTrendline, showResistanceTrend, debugSegmentOnly, height, onTrendlineResult, longtermLayer, longtermResistanceLayer, midtermLayer, shorttermLayer, activeLayer, activeResistanceLayer, horizontalZones, tieredSR])
