...
// ── Accelerated Trendlines (motor acelerado) ────────────────────────────────

export interface AccelTrendlineTier {
  ...
  stop_hint:          { side: string; value: number; method: string }
}

export interface AccelTrendlinesResponse {
  symbol:         string
  timeframe_used: string
  price:          number | null
  support:        AccelTrendlineTier | null
  resistance:     AccelTrendlineTier | null
  regime:         'accelerating_up' | 'accelerating_down' | 'compression' | 'none'
  meta:           {
    bars?:                 number
    pivot_lows?:           number
    pivot_highs?:          number
    rejected_candidates?:  any[] | null
    reason?:               string
  }
}
...
export const api = {
  ...
  ohlcvExtended: (id: number, tf: 'D' | 'W' | 'M' = 'D') =>
    get<{ candles: Candle[]; tf: string; stale?: boolean; last_date?: string | null }>(`/assets/${id}/ohlcv-extended?tf=${tf}`),
  ...
  elliott: (id: number, tf: 'D' | 'W' = 'D') =>
    get<ElliottResult>(`/assets/${id}/elliott?tf=${tf}`),

  elliottScan: (market = 'USA', tf: 'D' | 'W' = 'D') =>
    get<{ items: ElliottScanItem[]; fecha: string; market: string }>(
      `/scan/elliott?market=${market}&tf=${tf}`
    ),
  ...
  acceleratedTrendlines: (id: number, tf: 'D' | 'W' = 'D') =>
    get<AccelTrendlinesResponse>(`/assets/${id}/accelerated-trendlines?tf=${tf}`),

  cryptoPrices: () =>
    get<{ coins: CryptoCoin[]; ts: string }>('/crypto/prices'),

  cryptoOhlcv: (coinId: string, days = 90) =>
    get<{ candles: CryptoCandle[] }>(`/crypto/${coinId}/ohlcv?days=${days}`),

  cryptoAlerts: (coinId?: string) =>
    get<{ alerts: CryptoAlert[] }>(`/crypto/alerts${coinId ? `?coin_id=${coinId}` : ''}`),
...
export async function createCryptoAlert(input: { coin_id: string; direction: string; target_price: number; label?: string }) {
  ...
  const data = await res.json()
  if (!res.ok) throw new Error(data.error || 'Error al crear alerta crypto')
  return data as { ok: boolean; id: number }
}
...
export async function deleteCryptoAlert(id: number) {
  ...
  const data = await res.json()
  if (!res.ok) throw new Error(data.error || 'Error al eliminar alerta crypto')
  return data as { ok: boolean }
}
